...
import { Brain, HeartHandshake, Check, Zap, Eye, Timer, Heart, ArrowRight } from 'lucide-react';
    ...
    <div className="min-h-screen bg-white" dir={lang === 'ar' ? 'rtl' : 'ltr'}>
      ...
      <nav className="flex items-center justify-between px-6 py-4 max-w-5xl mx-auto">
        <span className="text-xl font-black text-teal-600 inline-flex items-center gap-2" style={{ fontFamily: 'DM Sans' }}>
          <Brain size={22} strokeWidth={2} className="text-teal-600" /> FocusBrain
        </span>
        <div className="flex items-center gap-3">
          ...
          <Link to="/donate" className="text-violet-600 font-bold text-sm px-4 py-2 rounded-xl hover:bg-violet-50 transition-colors flex items-center gap-1.5">
      ...
      <section className="max-w-3xl mx-auto text-center px-6 py-20">
        <motion.div {...fade}>
          ...
          <h1 className="text-5xl font-black text-ink-900 leading-tight mb-6" style={{ fontFamily: 'DM Sans' }}>
            {l.hero1}<br />
            <span className="text-teal-500">{l.hero2}</span><br />
            {l.hero3}
          </h1>
          <p className="text-xl text-ink-500 mb-10 max-w-xl mx-auto leading-relaxed">
            {l.sub}
          </p>
      ...
      <section className="bg-teal-500 py-20 text-center text-white">
        <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}>
          ...
          <Link
            to="/register"
            className="inline-flex items-center gap-2 bg-white text-teal-600 font-black text-xl px-12 py-4 rounded-2xl hover:bg-surface-soft transition-colors shadow-card"
          >
            {l.cta}
            <ArrowRight size={20} strokeWidth={2.5} />
          </Link>
