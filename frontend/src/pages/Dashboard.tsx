...
import {
  ...
  BarChart3, CheckCircle2, Brain, Hourglass, Target, Calendar, CalendarDays,
  Clock, Timer, Zap, Repeat, Star, PartyPopper, Video, AlertTriangle,
  Send, Hand, MessageSquare, Save, Loader2, Ban,
} from 'lucide-react';
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    ...
    CANCELLED: { label: '● Annulé',     cls: 'bg-surface-muted text-ink-400' },
  };
...
export default function Dashboard() {
  ...
  const metaLabel = (s: any) => {
    ...
    return [c && c.label, a && a.label, e && e.label].filter(Boolean).join('  ·  ');
  };
  ...
  return (
    <div className="flex h-full bg-surface-soft overflow-hidden">
      ...
      {/* ── COLONNE GAUCHE ──────────────────────────────────────────────────── */}
      <aside className="w-72 bg-white border-r border-line flex flex-col overflow-y-auto shrink-0">
        {/* Header user */}
        <div className="p-5 border-b border-line">
          <h2 className="font-black text-ink-900 flex items-center gap-1.5">
            Bonjour {user?.name?.split(' ')[0]}
            <Hand size={16} strokeWidth={2} className="text-amber-400" />
          </h2>
        ...
        {kpis && (
          <div className="px-4 pt-4">
            <p className="text-xs font-bold text-ink-400 uppercase tracking-wide mb-2 flex items-center gap-1.5">
              <BarChart3 size={14} strokeWidth={2} /> Indicateurs
            </p>
            <div className="grid grid-cols-3 gap-2">
              <div className="bg-teal-50 rounded-xl p-2 text-center">
                ...
                <p className="text-[10px] text-ink-500 leading-tight">1ʳᵉ session</p>
              </div>
              <div className="bg-teal-50 rounded-xl p-2 text-center">
                <p className="text-base font-black text-teal-700">{kpis.completionRate ?? 0}%</p>
                <p className="text-[10px] text-ink-500 leading-tight">complétion</p>
              </div>
              <div className="bg-amber-50 rounded-xl p-2 text-center">
                <p className="text-base font-black text-amber-700">{kpis.noShowRate ?? 0}%</p>
                <p className="text-[10px] text-ink-500 leading-tight">no-show</p>
              </div>
            </div>
            {kpis.me && (
              <p className="text-[11px] text-ink-400 mt-2 text-center inline-flex items-center justify-center gap-1 w-full">
                Toi : {kpis.me.completed} session(s)
                <CheckCircle2 size={13} strokeWidth={2} className="text-teal-600" />
                · {reliability({ sessionsCompleted: kpis.me.completed, sessionsNoShow: kpis.me.noShow }).label}
              </p>
            )}
          </div>
        )}
        ...
        {/* Sessions confirmées */}
        <div className="p-4">
          <p className="text-xs font-bold text-ink-400 uppercase tracking-wide mb-3 flex items-center gap-1.5">
            <CheckCircle2 size={14} strokeWidth={2} className="text-teal-600" /> Sessions confirmées ({confirmed.length})
          </p>
          {confirmed.length === 0
            ? <p className="text-xs text-ink-400 italic">Aucune session confirmée</p>
            : confirmed.map((s: any) => {
                    ...
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex items-center gap-2 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="font-bold text-sm text-ink-900 truncate">{partner?.name || 'Partenaire'}</p>
                      ...
                      <div className="flex items-center gap-0.5 shrink-0">
                        <button onClick={() => setDetailSlot(s)} title="Détails" className="p-1.5 text-ink-400 hover:text-teal-600 hover:bg-teal-100 rounded-lg transition-colors"><Eye size={15} /></button>
                        <button onClick={() => openEditModal(s)} title="Modifier" className="p-1.5 text-ink-400 hover:text-teal-600 hover:bg-teal-100 rounded-lg transition-colors"><Pencil size={15} /></button>
                        <button onClick={() => setCancelTarget(s.id)} title="Annuler la session" className="p-1.5 text-ink-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={15} /></button>
                      </div>
                    </div>
                    {/* Fiabilité + favori + agenda */}
                    <div className="flex items-center gap-2 mb-2 flex-wrap text-[11px]">
                      <span className="text-teal-700 font-semibold">{reliability(partner).label}</span>
                      <button onClick={() => toggleFavorite(partner?.id)} title="Partenaire favori" className="leading-none text-ink-400 hover:text-amber-400">
                        <Star size={14} strokeWidth={2} className={isFavorite(partner?.id) ? 'text-amber-400 fill-amber-400' : ''} />
                      </button>
                      <a href={googleCalendarUrl({ title: `Focus avec ${partner?.name || 'partenaire'}`, start: new Date(s.startTime), durationMin: s.duration, details: s.creatorTask || '' })}
                        target="_blank" rel="noreferrer" className="text-teal-700 hover:underline inline-flex items-center gap-1">
                        <CalendarDays size={13} strokeWidth={2} /> Agenda
                      </a>
                    </div>
                    <button
                      onClick={() => navigate(`/live/${s.id}`)}
                      className="w-full py-2 rounded-xl text-xs font-black transition-colors bg-teal-500 text-white hover:bg-teal-600 inline-flex items-center justify-center gap-1.5"
                    >
                      <Video size={15} strokeWidth={2} /> Rejoindre le Live
                    </button>
                  </motion.div>
                );
              })
        ...
        <div className="p-4 border-t border-line">
          <p className="text-xs font-bold text-ink-400 uppercase tracking-wide mb-3 flex items-center gap-1.5">
            <Brain size={14} strokeWidth={2} /> Mes créneaux ({created.length})
          </p>
          {created.length === 0
            ? <p className="text-xs text-ink-400 italic">Clique sur le calendrier pour créer un créneau</p>
            : created.map((s: any) => {
                ...
                return (
                  <motion.div key={s.id} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }}
                    className="bg-white border border-line rounded-xl p-3 mb-2">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        <button onClick={() => setDetailSlot(s)} title="Détails"
                          className="p-1.5 text-ink-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors"><Eye size={15} /></button>
                        <button onClick={() => openEditModal(s)} title="Modifier"
                          className="p-1.5 text-ink-400 hover:text-teal-600 hover:bg-teal-50 rounded-lg transition-colors"><Pencil size={15} /></button>
                        <button onClick={() => setDeleteTarget(s.id)} title="Supprimer"
                          className="p-1.5 text-ink-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={15} /></button>
                      </div>
                    </div>
                    ...
                    {slotTasks(s).length > 0 && (
                      <div className="mb-1 space-y-1">
                        {slotTasks(s).map((t: string, i: number) => (
                          <p key={i} className="text-xs text-teal-700 bg-teal-50 rounded-lg px-2 py-1 flex items-center gap-1.5">
                            <Target size={13} strokeWidth={2} className="shrink-0" /> {t}
                          </p>
                        ))}
                      </div>
                    )}
                    {metaLabel(s) && <p className="text-[11px] text-ink-400 mb-2">{metaLabel(s)}</p>}
                    ...
                    {slotCandidates.length > 0 && (
                      <div className="mb-2 space-y-1">
                        ...
                        {(s.requests || []).map((r: any) => (
                          <div key={r.user?.id || r.id} className="bg-surface-soft rounded-xl px-3 py-2 border border-line">
                              ...
                              <div className="flex items-center gap-1.5">
                                ...
                                <div>
                                  <p className="text-xs font-bold text-ink-700 flex items-center gap-1">
                                    {r.user?.name}
                                    <button onClick={() => toggleFavorite(r.user?.id)} title="Favori" className="leading-none text-ink-400 hover:text-amber-400">
                                      <Star size={12} strokeWidth={2} className={isFavorite(r.user?.id) ? 'text-amber-400 fill-amber-400' : ''} />
                                    </button>
                                  </p>
                                  <p className="text-[10px] text-ink-400">
                                    {reliability(r.user).label}{r.user?.tdahType ? ` · ${r.user.tdahType.replace(/_/g, ' ')}` : ''}
                                  </p>
                                </div>
                              </div>
                              <button
                                ...
                                className="text-xs bg-teal-500 text-white px-2.5 py-1 rounded-lg hover:bg-teal-600 disabled:opacity-40 font-bold inline-flex items-center gap-1"
                              >
                                <CheckCircle2 size={13} strokeWidth={2} /> Choisir
                              </button>
                            </div>
                            {/* Tâche du candidat */}
                            {r.candidateTask && (
                              <p className="text-[11px] text-ink-500 bg-white rounded-lg px-2 py-1 border border-line mt-1 flex items-center gap-1.5">
                                <Target size={12} strokeWidth={2} className="shrink-0" /> "{r.candidateTask}"
                              </p>
                            )}
                          </div>
                        ))}
        ...
        {pending.length > 0 && (
          <div className="p-4 border-t border-line">
            <p className="text-xs font-bold text-ink-400 uppercase tracking-wide mb-3 flex items-center gap-1.5">
              <Hourglass size={14} strokeWidth={2} /> J'attends une réponse ({pending.length})
            </p>
            {pending.map((r: any) => (
              <div key={r.id} className="bg-amber-50 border border-amber-100 rounded-xl p-3 mb-2">
                ...
                <p className="text-xs text-ink-500 mt-0.5">Par {r.slot.creator.name}</p>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {kpis && kpis.me && kpis.me.completed === 0 && (
          <div className="bg-amber-50 border-b border-amber-200 px-6 py-2 text-center text-sm text-amber-800 font-semibold flex items-center justify-center gap-1.5">
            <Hand size={15} strokeWidth={2} /> Bienvenue ! Lance ta 1ʳᵉ session de focus en 1 clic ci-dessous
          </div>
        )}
        ...
        <div className="bg-white border-b border-line px-6 py-3 flex items-center justify-between">
          <div>
            <h1 className="font-black text-ink-900 text-lg">{sc.title}</h1>
            <div className="flex gap-4 mt-1">
              {[
                ...
                <span key={l.label} className="flex items-center gap-1.5 text-xs text-ink-500">
        ...
        <div className="flex-1 overflow-auto p-4">
          <div className="h-full bg-white rounded-2xl border border-line overflow-hidden" style={{ minHeight: 500 }}>
            <FullCalendar
              ref={calendarRef}
      ...
      <AnimatePresence>
        {createModal && (
          <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <motion.div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-card max-h-[92vh] overflow-y-auto"
              initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }}
              onClick={e => e.stopPropagation()}>
              <h3 className="font-black text-ink-900 text-xl mb-3">{sc.createTitle}</h3>

              {/* Type de session */}
              <div className="grid grid-cols-3 gap-2 mb-3">
                {([
                  { id: 'INSTANT',   label: sc.typeInstant,   Icon: Zap },
                  { id: 'SCHEDULED', label: sc.typeScheduled, Icon: Calendar },
                  { id: 'RECURRING', label: sc.typeRecurring, Icon: Repeat },
                ] as const).map(ty => (
                  <button key={ty.id} onClick={() => setSlotType(ty.id)}
                    className={`py-2 rounded-xl text-xs font-bold transition-colors inline-flex items-center justify-center gap-1 ${slotType === ty.id ? 'bg-teal-500 text-white' : 'bg-surface-muted text-ink-500 hover:bg-surface-soft'}`}>
                    <ty.Icon size={14} strokeWidth={2} /> {ty.label}
                  </button>
                ))}
              </div>
              ...
              {slotType === 'INSTANT' ? (
                ...
                <div className="mb-5">
                  <p className="text-sm font-bold text-ink-700 mb-2 flex items-center gap-1.5">
                    <Calendar size={15} strokeWidth={2} /> {slotType === 'RECURRING' ? '1ʳᵉ session — date et heure' : 'Date et heure de démarrage'}
                  </p>
                  <div className="grid grid-cols-2 gap-2">
                    <input type="date" value={createDate} onChange={e => setCreateDate(e.target.value)}
                      className="w-full border-2 border-line focus:border-teal-400 rounded-xl px-3 py-2.5 text-sm outline-none" />
                    <input type="time" step={60} value={createTime} onChange={e => setCreateTime(e.target.value)}
                      className="w-full border-2 border-line focus:border-teal-400 rounded-xl px-3 py-2.5 text-sm outline-none" />
                  </div>
                  {createDate && createTime && (
                    <p className="text-xs text-ink-500 mt-1.5 capitalize">
                  ...
                  {createDate && createTime && createStart().getTime() < Date.now() && (
                    <p className="text-xs text-amber-600 mt-1 flex items-center gap-1.5">
                      <AlertTriangle size={13} strokeWidth={2} /> Cette date est déjà passée — choisis un horaire à venir.
                    </p>
                  )}
                </div>
              )}
              ...
              {slotType === 'RECURRING' && (
                <div className="bg-violet-50 border border-violet-100 rounded-xl p-3 mb-5">
                  <p className="text-xs font-bold text-violet-700 mb-2">{sc.repetition}</p>
                  <div className="grid grid-cols-2 gap-2 mb-3">
                    {([{ id: 'DAILY', label: sc.daily }, { id: 'WEEKLY', label: sc.weekly }] as const).map(f => (
                      <button key={f.id} onClick={() => setRecFreq(f.id)}
                        className={`py-2 rounded-lg text-xs font-bold transition-colors ${recFreq === f.id ? 'bg-violet-500 text-white' : 'bg-white text-ink-500 border border-violet-200'}`}>
                        {f.label}
                      </button>
                    ))}
                  </div>
                    ...
                    <div className="flex gap-1 mb-3">
                      {['D', 'L', 'M', 'M', 'J', 'V', 'S'].map((d, i) => (
                        <button key={i} onClick={() => setRecDays(p => p.includes(i) ? p.filter(x => x !== i) : [...p, i])}
                          className={`flex-1 py-1.5 rounded-lg text-xs font-bold ${recDays.includes(i) ? 'bg-violet-500 text-white' : 'bg-white text-ink-500 border border-violet-200'}`}>
                          {d}
                        </button>
                      ))}
                    </div>
                  )}
                  <label className="text-xs font-bold text-violet-700">{sc.occurrences} : {recCount}</label>
              ...
              <p className="text-sm font-bold text-ink-700 mb-3">Durée de la session</p>
              <div className="grid grid-cols-4 gap-2 mb-5">
                {DURATIONS.map(d => (
                  <button key={d} onClick={() => setDuration(d)}
                    className={`py-3 rounded-xl text-sm font-black transition-colors ${
                      duration === d ? 'bg-teal-500 text-white' : 'bg-surface-muted text-ink-500 hover:bg-surface-soft'
                    }`}>
                    {d}min
                  </button>
                ))}
              ...
              {/* Catégorie de tâche */}
              <p className="text-sm font-bold text-ink-700 mb-2">Catégorie</p>
              <div className="grid grid-cols-3 gap-2 mb-4">
                {TASK_CATEGORIES.map(c => (
                  <button key={c.id} onClick={() => setSlotCategory(c.id)}
                    className={`py-2 rounded-xl text-xs font-bold transition-colors ${slotCategory === c.id ? 'bg-teal-500 text-white' : 'bg-surface-muted text-ink-500 hover:bg-surface-soft'}`}>
                    {c.label}
                  </button>
                ))}
              </div>
              ...
              {/* Plusieurs tâches */}
              <p className="text-sm font-bold text-ink-700 mb-2 flex items-center gap-1.5">
                <Target size={15} strokeWidth={2} /> Tes tâches <span className="text-ink-400 font-normal ml-1">(plusieurs possibles)</span>
              </p>
              <div className="flex gap-2 mb-2">
                <input
                  ...
                  maxLength={200}
                  className="flex-1 border-2 border-line focus:border-teal-400 rounded-xl px-4 py-2.5 text-sm outline-none transition-colors"
                />
                <button onClick={addSlotTask} disabled={!creatorTask.trim() || taskList.length >= 8}
                  className="bg-teal-100 text-teal-700 font-black px-4 rounded-xl hover:bg-teal-200 disabled:opacity-40 inline-flex items-center">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {taskList.map((t, i) => (
                    <span key={i} className="inline-flex items-center gap-1 bg-teal-50 text-teal-800 text-xs font-semibold rounded-lg px-2 py-1">
                      {i + 1}. {t}
                      <button onClick={() => setTaskList(p => p.filter((_, j) => j !== i))} className="text-teal-400 hover:text-red-500 ml-0.5 inline-flex items-center">
                        <Plus size={13} strokeWidth={2} className="rotate-45" />
                      </button>
                    </span>
                  ))}
                </div>
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {AMBIANCES.map(a => (
                  <button key={a.id} onClick={() => setAmbiance(a.id)}
                    className={`py-2 rounded-xl text-xs font-bold transition-colors ${ambiance === a.id ? 'bg-teal-500 text-white' : 'bg-surface-muted text-ink-500 hover:bg-surface-soft'}`}>
                    {a.label}
                  </button>
                ))}
              </div>
              ...
              {/* Niveau d'énergie */}
              <p className="text-sm font-bold text-ink-700 mb-2">Niveau d'énergie</p>
              <div className="grid grid-cols-3 gap-2 mb-5">
                {ENERGIES.map(en => (
                  <button key={en.id} onClick={() => setEnergy(en.id)}
                    className={`py-2 rounded-xl text-xs font-bold transition-colors ${energy === en.id ? 'bg-teal-500 text-white' : 'bg-surface-muted text-ink-500 hover:bg-surface-soft'}`}>
                    {en.label}
                  </button>
                ))}
              </div>
      ...
      <AnimatePresence>
        {editModal && (
          <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <motion.div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-card"
              initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
              onClick={e => e.stopPropagation()}>
              <h3 className="font-black text-ink-900 text-xl mb-1 flex items-center gap-1.5">
                <Pencil size={18} strokeWidth={2} /> Modifier la session
              </h3>
              <p className="text-xs text-ink-400 mb-5">{editModal?.status === 'CONFIRMED' ? 'Ton partenaire sera notifié du changement.' : 'Les candidats seront notifiés du changement.'}</p>

              {/* Nouvelle date/heure */}
              <p className="text-sm font-bold text-ink-700 mb-2 flex items-center gap-1.5"><Calendar size={15} strokeWidth={2} /> Date et heure</p>
              <input
                type="datetime-local"
                value={editDate}
                onChange={e => setEditDate(e.target.value)}
                className="w-full border-2 border-line focus:border-teal-400 rounded-xl px-4 py-2.5 text-sm outline-none mb-4"
              />

              {/* Durée */}
              <p className="text-sm font-bold text-ink-700 mb-2 flex items-center gap-1.5"><Timer size={15} strokeWidth={2} /> Durée</p>
              <div className="grid grid-cols-4 gap-2 mb-4">
                {[15, 25, 50, 75].map(d => (
                  <button key={d} onClick={() => setEditDuration(d)}
                    className={`py-2.5 rounded-xl text-sm font-black transition-colors ${
                      editDuration === d ? 'bg-teal-500 text-white' : 'bg-surface-muted text-ink-500 hover:bg-surface-soft'
                    }`}>
                    {d}min
                  </button>
                ))}
              ...
              {/* Tâche */}
              <p className="text-sm font-bold text-ink-700 mb-2 flex items-center gap-1.5">
                <Target size={15} strokeWidth={2} /> Ta tâche <span className="font-normal text-ink-400">(optionnel)</span>
              </p>
              <input
                value={editTask}
              ...
              {/* Description */}
              <p className="text-sm font-bold text-ink-700 mb-2 flex items-center gap-1.5">
                <MessageSquare size={15} strokeWidth={2} /> Description <span className="font-normal text-ink-400">(optionnel)</span>
              </p>
              <textarea
                ...
                rows={2}
                className="w-full border-2 border-line focus:border-teal-400 rounded-xl px-4 py-2.5 text-sm outline-none mb-5 resize-none"
              />

              <div className="flex gap-3">
                <button onClick={() => setEditModal(null)}
                  className="flex-1 border-2 border-line text-ink-500 font-bold py-3 rounded-xl hover:bg-surface-soft text-sm">
                  Annuler
                </button>
                <button
                  ...
                  className="flex-1 bg-teal-500 hover:bg-teal-600 disabled:opacity-40 text-white font-black py-3 rounded-xl text-sm inline-flex items-center justify-center gap-1.5"
                >
                  {editSlot.isPending
                    ? <Loader2 size={16} strokeWidth={2} className="animate-spin" />
                    : <><Save size={16} strokeWidth={2} /> Enregistrer</>}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {deleteTarget && (
          <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <motion.div className="bg-white rounded-2xl p-6 w-full max-w-xs shadow-card text-center"
              ...
              <h3 className="font-black text-ink-900 text-lg mb-2">Supprimer ce créneau ?</h3>
              <p className="text-ink-400 text-sm mb-6">
                Les candidats seront notifiés de l'annulation.
              </p>
              <div className="flex gap-3">
                <button onClick={() => setDeleteTarget(null)}
                  className="flex-1 border-2 border-line text-ink-500 font-bold py-3 rounded-xl text-sm hover:bg-surface-soft">
                  Annuler
                </button>
                <button
                  onClick={() => deleteSlot.mutate(deleteTarget)}
                  disabled={deleteSlot.isPending}
                  className="flex-1 bg-red-500 hover:bg-red-600 disabled:opacity-40 text-white font-black py-3 rounded-xl text-sm inline-flex items-center justify-center gap-1.5"
                >
                  {deleteSlot.isPending ? <Loader2 size={16} strokeWidth={2} className="animate-spin" /> : 'Supprimer'}
                </button>
              </div>
            </motion.div>
          </motion.div>
        ...
        {cancelTarget && (
          <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <motion.div className="bg-white rounded-2xl p-6 w-full max-w-xs shadow-card text-center"
              ...
              <div className="flex justify-center mb-3">
                <span className="w-14 h-14 rounded-2xl bg-red-50 flex items-center justify-center">
                  <Ban size={28} strokeWidth={2} className="text-red-500" />
                </span>
              </div>
              <h3 className="font-black text-ink-900 text-lg mb-2">Annuler cette session ?</h3>
              <p className="text-ink-400 text-sm mb-6">Ton partenaire sera prévenu de l'annulation.</p>
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 bg-red-500 hover:bg-red-600 disabled:opacity-40 text-white font-black py-3 rounded-xl text-sm inline-flex items-center justify-center gap-1.5"
                >
                  {cancelSlot.isPending ? <Loader2 size={16} strokeWidth={2} className="animate-spin" /> : 'Annuler la session'}
                </button>
              </div>
            </motion.div>
          </motion.div>
      ...
      <AnimatePresence>
        {detailSlot && (() => {
          ...
          const start = new Date(s.startTime);
          const partner = s.creatorId === user?.id ? s.partner : s.creator;
          const TypeIcon = s.type === 'INSTANT' ? Zap : s.type === 'RECURRING' ? Repeat : Calendar;
          const typeLabel = s.type === 'INSTANT' ? 'Instantanée' : s.type === 'RECURRING' ? 'Récurrente' : 'Planifiée';
          return (
            <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              onClick={() => setDetailSlot(null)}>
              <motion.div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-card max-h-[90vh] overflow-y-auto"
                initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95 }}
                onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-3">
                  <h3 className="font-black text-ink-900 text-xl">Détails de la session</h3>
                  <StatusBadge status={s.status} />
                </div>

                <div className="space-y-2 text-sm text-ink-700 mb-4">
                  <p className="flex items-center gap-1.5">
                    <Calendar size={15} strokeWidth={2} className="text-ink-400 shrink-0" />
                    <span><strong>{start.toLocaleDateString('fr', { weekday: 'long', day: 'numeric', month: 'long' })}</strong> à {start.toLocaleTimeString('fr', { hour: '2-digit', minute: '2-digit' })}</span>
                  </p>
                  <p className="flex items-center gap-1.5">
                    <Timer size={15} strokeWidth={2} className="text-ink-400 shrink-0" />
                    <span>Durée : <strong>{s.duration} min</strong></span>
                    <span className="text-ink-400">·</span>
                    <TypeIcon size={14} strokeWidth={2} className="text-ink-400 shrink-0" /> {typeLabel}
                  </p>
                  {metaLabel(s) && <p className="text-ink-500">{metaLabel(s)}</p>}
                </div>

                {slotTasks(s).length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs font-bold text-ink-400 uppercase mb-1 flex items-center gap-1.5"><Target size={13} strokeWidth={2} /> Tâches</p>
                ...
                {s.description && (
                  <div className="mb-4">
                    <p className="text-xs font-bold text-ink-400 uppercase mb-1 flex items-center gap-1.5"><MessageSquare size={13} strokeWidth={2} /> Description</p>
                    <p className="text-sm text-ink-500 bg-surface-soft rounded-lg px-3 py-2 whitespace-pre-wrap">{s.description}</p>
                  </div>
                )}
                ...
                {s.status === 'CONFIRMED' && partner && (
                  <div className="mb-4 flex items-center gap-2 bg-teal-50 rounded-xl px-3 py-2">
                    ...
                    <div>
                      <p className="font-bold text-sm text-ink-900">{partner.name}</p>
                      <p className="text-xs text-ink-500">{reliability(partner).label}</p>
                    </div>
                  </div>
                )}

                {s.status === 'PENDING' && (s.requests?.length || 0) > 0 && (
                  <p className="text-sm text-amber-700 bg-amber-50 rounded-lg px-3 py-2 mb-4 flex items-center gap-1.5">
                    <Hourglass size={15} strokeWidth={2} className="shrink-0" /> {s.requests.length} candidat{s.requests.length > 1 ? 's' : ''} en attente — choisis ton partenaire sur la carte.
                  </p>
                )}
                ...
                {detailMessages.length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs font-bold text-ink-400 uppercase mb-2 flex items-center gap-1.5"><MessageSquare size={13} strokeWidth={2} /> Conversation ({detailMessages.length})</p>
                    <div className="space-y-1.5 max-h-52 overflow-y-auto bg-surface-soft rounded-xl p-2 border border-line">
                      {detailMessages.map((m: any) => {
                        const mine = m.from?.id === user?.id;
                        return (
                          <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] rounded-xl px-2.5 py-1.5 text-xs break-words ${mine ? 'bg-teal-500 text-white' : 'bg-white border border-line text-ink-700'}`}>
                              {!mine && <p className="text-[9px] font-bold opacity-60 mb-0.5">{m.from?.name}</p>}
                              <span className="whitespace-pre-wrap">{m.content}</span>
                              <span className={`block text-[8px] mt-0.5 ${mine ? 'text-white/60' : 'text-ink-400'}`}>
                ...
                <div className="flex gap-2">
                  {s.status === 'CONFIRMED' && (
                    <button onClick={() => { setDetailSlot(null); navigate(`/live/${s.id}`); }}
                      className="flex-1 bg-teal-500 hover:bg-teal-600 text-white font-black py-2.5 rounded-xl text-sm inline-flex items-center justify-center gap-1.5"><Video size={15} strokeWidth={2} /> Rejoindre</button>
                  )}
                  <button onClick={() => { setDetailSlot(null); openEditModal(s); }}
                    className="flex-1 border-2 border-line text-ink-500 font-bold py-2.5 rounded-xl text-sm hover:bg-surface-soft flex items-center justify-center gap-1"><Pencil size={14} /> Modifier</button>
        ...
        {joinModal && (
          <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <motion.div className="bg-white rounded-2xl p-6 w-full max-w-sm shadow-card"
              initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }}
              onClick={e => e.stopPropagation()}>
              <h3 className="font-black text-ink-900 text-xl mb-4">Rejoindre ce créneau</h3>

              <div className="bg-surface-soft rounded-xl p-4 mb-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="font-black text-ink-900">{joinModal.creator?.name}</p>
                    <p className="text-xs text-ink-400">{joinModal.creator?.tdahType?.replace(/_/g, ' ') || 'TDAH'}</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-3 text-sm text-ink-500">
                  <span className="inline-flex items-center gap-1.5"><Calendar size={14} strokeWidth={2} className="text-ink-400" /> {new Date(joinModal.startTime).toLocaleDateString('fr', { weekday: 'short', day: 'numeric', month: 'short' })}</span>
                  <span className="inline-flex items-center gap-1.5"><Clock size={14} strokeWidth={2} className="text-ink-400" /> {new Date(joinModal.startTime).toLocaleTimeString('fr', { hour: '2-digit', minute: '2-digit' })}</span>
                  <span className="inline-flex items-center gap-1.5"><Timer size={14} strokeWidth={2} className="text-ink-400" /> {joinModal.duration}min</span>
                </div>
              </div>
              ...
              {(slotTasks(joinModal).length > 0 || metaLabel(joinModal)) && (
                <div className="bg-teal-50 border border-teal-100 rounded-xl px-4 py-3 mb-4">
                  <p className="text-xs font-bold text-teal-600 mb-1 flex items-center gap-1.5"><Target size={13} strokeWidth={2} /> {joinModal.creator?.name} va travailler sur :</p>
              ...
              {joinModal.status === 'PENDING' && (
                <div className="bg-amber-50 border border-amber-100 rounded-xl p-3 mb-4 text-sm text-amber-700 flex items-start gap-1.5">
                  <AlertTriangle size={15} strokeWidth={2} className="shrink-0 mt-0.5" /> D'autres personnes ont demandé ce créneau. Le créateur choisit son partenaire.
                </div>
              )}
              ...
              <div className="mb-4">
                <p className="text-sm font-bold text-ink-700 mb-2 flex items-center gap-1.5">
                  <Target size={15} strokeWidth={2} /> Et toi, sur quoi vas-tu travailler ?
                  <span className="text-ink-400 font-normal ml-1">(optionnel)</span>
                </p>
                <input
                  value={candidateTask}
                  ...
                  maxLength={200}
                  className="w-full border-2 border-line focus:border-teal-400 rounded-xl px-4 py-3 text-sm outline-none transition-colors"
                />
              </div>
              ...
              <button
                ...
                className="w-full bg-teal-500 text-white font-black py-4 rounded-xl hover:bg-teal-600 transition-colors disabled:opacity-50 text-base inline-flex items-center justify-center gap-2"
              >
                {requestSlot.isPending
                  ? <><Loader2 size={18} strokeWidth={2} className="animate-spin" /> Envoi...</>
                  : <><Send size={18} strokeWidth={2} /> Envoyer ma demande</>}
              </button>
              {requestSlot.isError && (
                <p className="text-red-500 text-xs mt-2 text-center">
      ...
      <AnimatePresence>
        {detailModal && (
          <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <motion.div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-card"
              initial={{ scale: 0.95, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 10 }}
              onClick={e => e.stopPropagation()}>
              <div className="flex items-center justify-between mb-2">
                <h3 className="font-black text-ink-900 text-xl">Mon créneau</h3>
                <StatusBadge status={detailModal.status} />
              </div>
              <p className="text-sm text-ink-500 mb-5 flex items-center gap-1.5">
                <Calendar size={14} strokeWidth={2} className="text-ink-400 shrink-0" />
              ...
              {detailModal.status === 'CONFIRMED' ? (
                <div className="bg-teal-50 rounded-xl p-5 text-center">
                  <div className="flex justify-center mb-2"><CheckCircle2 size={36} strokeWidth={2} className="text-teal-600" /></div>
                  ...
                  <button onClick={() => navigate(`/live/${detailModal.id}`)}
                    className="mt-4 bg-teal-500 text-white font-black px-6 py-3 rounded-xl hover:bg-teal-600 transition-colors inline-flex items-center justify-center gap-1.5">
                    <Video size={16} strokeWidth={2} /> Rejoindre le Live
                  </button>
                </div>
              ) : (detailModal.requests || []).length === 0 ? (
                <div className="text-center py-10 text-ink-400">
                  <div className="flex justify-center mb-3"><Hourglass size={40} strokeWidth={1.75} /></div>
                ...
                <div>
                  ...
                  <div className="space-y-2 max-h-72 overflow-y-auto">
                    {detailModal.requests.map((r: any) => (
                      <div key={r.id}
                        className="flex items-center justify-between bg-surface-soft rounded-xl px-4 py-3 hover:bg-teal-50 transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="w-10 h-10 bg-teal-100 rounded-full flex items-center justify-center font-black text-teal-600">
                            {r.user?.name?.[0]?.toUpperCase()}
                          </div>
                          <div>
                            <p className="font-bold text-sm text-ink-900">{r.user?.name}</p>
                            <p className="text-xs text-ink-400">{r.user?.tdahType?.replace(/_/g, ' ') || 'TDAH'}</p>
                          </div>
                        </div>
                        <button
                          onClick={() => confirmCandidate.mutate({ slotId: detailModal.id, candidateId: r.user.id })}
                          disabled={confirmCandidate.isPending}
                          className="bg-teal-500 text-white text-sm font-black px-4 py-2.5 rounded-xl hover:bg-teal-600 transition-colors disabled:opacity-50 inline-flex items-center gap-1.5"
                        >
                          Confirmer <CheckCircle2 size={15} strokeWidth={2} />
                        </button>
                      </div>
                    ))}
                  </div>
              ...
              <button onClick={() => setDetailModal(null)}
                className="mt-4 w-full text-ink-400 hover:text-ink-700 text-sm py-2 transition-colors">
                Fermer
              </button>
            </motion.div>
          </motion.div>
...
function FeedbackModal({ slot, currentUserId, onDone }: { slot: any; currentUserId?: string; onDone: () => void }) {
  ...
  return (
    <motion.div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4"
      initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <motion.div className="bg-white rounded-3xl p-6 w-full max-w-sm shadow-card"
        initial={{ scale: 0.94, y: 10 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.94 }}>
        <div className="text-center mb-4">
          <div className="flex justify-center mb-2"><PartyPopper size={36} strokeWidth={2} className="text-amber-400" /></div>
          <h3 className="font-black text-ink-900 text-lg">{sc.fbTitle}</h3>
        ...
        <div className="flex justify-center gap-1.5 mb-4">
          {[1, 2, 3, 4, 5].map(n => (
            <button key={n} onClick={() => setRating(n)} onMouseEnter={() => setHover(n)} onMouseLeave={() => setHover(0)}
              ...
              className={`transition-transform hover:scale-110 ${(hover || rating) >= n ? 'text-amber-400' : 'text-ink-400/40'}`}>
              <Star size={30} strokeWidth={2} className={(hover || rating) >= n ? 'fill-amber-400' : ''} />
            </button>
          ))}
        </div>
        ...
        <div className="flex justify-center gap-2 mb-4">
          {['😄', '🙂', '😐', '😕', '😴'].map(m => (
            <button key={m} onClick={() => setMood(m === mood ? '' : m)}
              className={`text-2xl w-10 h-10 rounded-xl transition-colors ${mood === m ? 'bg-teal-100' : 'hover:bg-surface-muted'}`}>
              {m}
            </button>
          ))}
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={submit} disabled={!rating || saving}
            className="flex-1 bg-teal-500 hover:bg-teal-600 disabled:opacity-40 text-white font-black py-3 rounded-xl text-sm inline-flex items-center justify-center gap-1.5">
            {saving ? <Loader2 size={16} strokeWidth={2} className="animate-spin" /> : sc.fbSend}
          </button>
        </div>
      </motion.div>
    </motion.div>
